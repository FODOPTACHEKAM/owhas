...
`ServerConfig.detect()` found no local IP and fell through to `owhas.org`.
...
> *"Cloud server (owhas.org) is not reachable. Check your internet connection
...
Both the local hotspot server and `owhas.org` are running simultaneously
under the same PIN.
